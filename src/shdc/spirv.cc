...
namespace shdc
{
    ...
    static MergedSource merge_source(const Input& inp, const Snippet& snippet, Slang::Enum slang,
    ...
    static void infolog_to_errors(const std::string& log, const Input& inp, int snippet_index, int linenr_offset,
                                  std::vector<ErrMsg>& out_errors)
    {
        /*
    ...
    */
    static void spirv_optimize(Slang::Enum slang, std::vector<uint32_t>& spirv)
    {
        ...
        */
        ...
        // NOTE: call the "preserveInterface" version of CreateAggressiveDCEPass()
    ...
    static bool compile(Input& inp, EShLanguage stage, Slang::Enum slang, const MergedSource& source, int snippet_index,
                        Spirv& out_spirv)
    {
        ...
        for (int i = 0; i < program.getNumUniformBlocks(); i++)
        {
            ...
            {
                out_spirv.errors.push_back(inp.error(
                    0, fmt::format("different bindings for uniform blocks of same name '{}' ({} vs {})", name, slot,
                                   binding)));
                return false;
            }
        }
        for (int i = 0; i < program.getNumBufferBlocks(); i++)
        {
            ...
            {
                out_spirv.errors.push_back(inp.error(
                    0, fmt::format("different bindings for buffer blocks of same name '{}' ({} vs {})", name, slot,
                                   binding)));
                return false;
            }
        }
        for (int i = 0; i < program.getNumUniformVariables(); i++)
        {
            ...
            if (uniform.getType()->getSampler().sampler)
            {
                ...
                {
                    out_spirv.errors.push_back(inp.error(
                        0, fmt::format("different bindings for samplers of same name '{}' ({} vs {})", name, slot,
                                       binding)));
                    return false;
                }
            }
            else if (uniform.getType()->isTexture())
            {
                ...
                {
                    out_spirv.errors.push_back(inp.error(
                        0, fmt::format("different bindings for textures of same name '{}' ({} vs {})", name, slot,
                                       binding)));
                    return false;
                }
            }
    ...
    Spirv Spirv::compile_glsl_and_extract_bindings(Input& inp, Slang::Enum slang,
                                                   const std::vector<std::string>& defines)
    {
        Spirv out_spirv;
