...
namespace shdc::gen
{
        ...
        for (const Type& uniform : ub.struct_info.struct_items)
        {
            ...
            else
            {
                ...
                else
                {
                    switch (uniform.type)
                    {
                    case Type::Float4: l("float[{}][4] {};\n", uniform.array_count, uniform.name);
                        break;
                    case Type::Int4: l("int[{}][4] {};\n", uniform.array_count, uniform.name);
                        break;
                    case Type::Mat4x4: l("float[{}][16] {};\n", uniform.array_count, uniform.name);
    ...
    void SokolC3Generator::gen_shader_desc_func(const GenInput& gen, const ProgramReflection& prog)
    {
        ...
        for (int i = 0; i < Slang::Num; i++)
        {
            ...
            if (gen.args.slang & Slang::bit(slang))
            {
                ...
                {
                    const ShaderStageArrayInfo& info = shader_stage_array_info(
                        gen, prog, ShaderStage::from_index(stage_index), slang);
                    const StageReflection& refl = prog.stages[stage_index];
                    const std::string dsn = fmt::format(
                        "desc.{}", info.stage == ShaderStage::Vertex ? "vertex_func" : "fragment_func");
                ...
                if (Slang::is_glsl(slang))
                {
                    l("desc.spv_vs.ptr = glsl_spv_vs;\n");
                    l("desc.spv_vs.size = sizeof(glsl_spv_vs);\n");
                    l("desc.spv_fs.ptr = glsl_spv_fs;\n");
                    l("desc.spv_fs.size = sizeof(glsl_spv_fs);\n");
                }
                ...
                for (int img_smp_index = 0; img_smp_index < Bindings::MaxImageSamplers; img_smp_index++)
                {
                    ...
                    if (img_smp)
                    {
                        ...
                        l("{}.image_slot = {};\n", isn,
                          prog.bindings.find_image_by_name(img_smp->image_name)->sokol_slot);
                        l("{}.sampler_slot = {};\n", isn,
                          prog.bindings.find_sampler_by_name(img_smp->sampler_name)->sokol_slot);
                        if (Slang::is_glsl(slang))
                        {
                            l("{}.glsl_name = \"{}\";\n", isn, img_smp->name);
                        }
    ...
    void SokolC3Generator::gen_shader_array_start(const GenInput& gen, const std::string& array_name, size_t num_bytes,
                                                  Slang::Enum slang)
    {
