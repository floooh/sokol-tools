...
void SokolC2Generator::gen_prolog(const GenInput& gen) {
    ...
    l("import sokol_gfx as sg;\n");
}

void SokolC2Generator::gen_epilog(const GenInput& gen) {
    // empty
}

void SokolC2Generator::gen_prerequisites(const GenInput& gen) {
    // empty
}
        ...
        if (gen.inp.ctype_map.count(uniform.type_as_glsl()) > 0) {
            ...
            if (uniform.array_count == 0) {
                switch (uniform.type) {
                    ...
                    case Type::Float2:  l("f32[2] {};\n", uniform.name); break;
                    case Type::Float3:  l("f32[3] {};\n", uniform.name); break;
                    case Type::Float4:  l("f32[4] {};\n", uniform.name); break;
                    case Type::Int:     l("i32 {};\n", uniform.name); break;
                    case Type::Int2:    l("i32[2] {};\n", uniform.name); break;
                    case Type::Int3:    l("i32[3] {};\n", uniform.name); break;
                    case Type::Int4:    l("i32[4] {};\n", uniform.name); break;
                    case Type::Mat4x4:  l("f32[16] {};\n", uniform.name); break;
...
void SokolC2Generator::gen_struct_interior_decl_std430(const GenInput& gen, const Type& struc, int pad_to_size) {
    ...
    for (const Type& item: struc.struct_items) {
        ...
        if (item.type == Type::Struct) {
            if (item.array_count == 0) {
                l("{} {};\n", struc.struct_typename, item.name);
            } else {
                l("{}[{}] {};\n", struc.struct_typename, item.array_count, item.name);
            }
            ...
            if (item.array_count == 0) {
                switch (item.type) {
                    ...
                    case Type::Bool2:   l("i32[2] {};\n", item.name); break;
                    case Type::Bool3:   l("i32[3] {};\n", item.name); break;
                    case Type::Bool4:   l("i32[4] {};\n", item.name); break;
                    case Type::Int:     l("i32 {};\n", item.name); break;
                    case Type::Int2:    l("i32[2] {};\n", item.name); break;
                    case Type::Int3:    l("i32[3] {};\n", item.name); break;
                    case Type::Int4:    l("i32[4] {};\n", item.name); break;
                    case Type::UInt:    l("u32 {};\n", item.name); break;
                    case Type::UInt2:   l("u32[2] {};\n", item.name); break;
                    case Type::UInt3:   l("u32[3] {};\n", item.name); break;
                    case Type::UInt4:   l("u32[4] {};\n", item.name); break;
                    case Type::Float:   l("f32 {};\n", item.name); break;
                    case Type::Float2:  l("f32[2] {};\n", item.name); break;
                    case Type::Float3:  l("f32[3] {};\n", item.name); break;
                    case Type::Float4:  l("f32[4] {};\n", item.name); break;
                    case Type::Mat2x1:  l("f32[2] {};\n", item.name); break;
                    case Type::Mat2x2:  l("f32[4] {};\n", item.name); break;
                    case Type::Mat2x3:  l("f32[6] {};\n", item.name); break;
                    case Type::Mat2x4:  l("f32[8] {};\n", item.name); break;
                    case Type::Mat3x1:  l("f32[3] {};\n", item.name); break;
                    case Type::Mat3x2:  l("f32[6] {};\n", item.name); break;
                    case Type::Mat3x3:  l("f32[9] {};\n", item.name); break;
                    case Type::Mat3x4:  l("f32[12] {};\n", item.name); break;
                    case Type::Mat4x1:  l("f32[4] {};\n", item.name); break;
                    case Type::Mat4x2:  l("f32[8] {};\n", item.name); break;
                    case Type::Mat4x3:  l("f32[12] {};\n", item.name); break;
                    case Type::Mat4x4:  l("f32[16] {};\n", item.name); break;
...
void SokolC2Generator::gen_shader_array_start(const GenInput& gen, const std::string& array_name, size_t num_bytes, Slang::Enum slang) {
    l("const char[{}] {} = {{\n", num_bytes, array_name);
}

void SokolC2Generator::gen_shader_array_end(const GenInput& gen) {
    l("\n}}\n");
}

std::string SokolC2Generator::lang_name() {
    return "C2";
}

std::string SokolC2Generator::comment_block_start() {
    return "/*";
}

std::string SokolC2Generator::comment_block_end() {
    return "*/";
}

std::string SokolC2Generator::comment_block_line_prefix() {
    return "";
}

std::string SokolC2Generator::shader_bytecode_array_name(const std::string& snippet_name, Slang::Enum slang) {
    return pystring::upper(fmt::format("{}_bytecode_{}", snippet_name, Slang::to_str(slang)));
}

std::string SokolC2Generator::shader_source_array_name(const std::string& snippet_name, Slang::Enum slang) {
    return pystring::upper(fmt::format("{}_source_{}", snippet_name, Slang::to_str(slang)));
}

std::string SokolC2Generator::get_shader_desc_help(const std::string& prog_name) {
...
std::string SokolC2Generator::attr_basetype(Type::Enum e) {
    switch (e) {
...
std::string SokolC2Generator::flattened_uniform_type(Type::Enum e) {
    switch (e) {
        case Type::Float:
        case Type::Float2:
        case Type::Float3:
...
std::string SokolC2Generator::image_type(ImageType::Enum e) {
    switch (e) {
        //case ImageType::_2D:     return "sg.ImageType.TYPE_2D";
        case ImageType::_2D:     return "TYPE_2D";
        case ImageType::CUBE:    return "CUBE";
        case ImageType::_3D:     return "TYPE_3D";
        case ImageType::ARRAY:   return "ARRAY";
        default: return "INVALID";
    }
}

std::string SokolC2Generator::image_sample_type(ImageSampleType::Enum e) {
    switch (e) {
        ...
        case ImageSampleType::UNFILTERABLE_FLOAT:  return "sg.ImageSampleType.UNFILTERABLE_FLOAT";
        default: return "INVALID";
    }
}
...
std::string SokolC2Generator::backend(Slang::Enum e) {
    switch (e) {
        case Slang::GLSL410:
        case Slang::GLSL430:
            return "GLCORE";
...
std::string SokolC2Generator::struct_name(const std::string& name) {
    return to_pascal_case(name);
}

std::string SokolC2Generator::vertex_attr_name(const std::string& prog_name, const StageAttr& attr) {
    return pystring::upper(fmt::format("ATTR_{}_{}", prog_name, attr.name));
}

std::string SokolC2Generator::texture_bind_slot_name(const Texture& tex) {
    return pystring::upper(fmt::format("VIEW_{}", tex.name));
}

std::string SokolC2Generator::storage_buffer_bind_slot_name(const StorageBuffer& sbuf) {
    return pystring::upper(fmt::format("VIEW_{}", sbuf.name));
}

std::string SokolC2Generator::storage_image_bind_slot_name(const StorageImage& simg) {
    return pystring::upper(fmt::format("VIEW_{}", simg.name));
}

std::string SokolC2Generator::sampler_bind_slot_name(const Sampler& smp) {
    return pystring::upper(fmt::format("SMP_{}", smp.name));
}

std::string SokolC2Generator::uniform_block_bind_slot_name(const UniformBlock& ub) {
    return pystring::upper(fmt::format("UB_{}", ub.name));
}

std::string SokolC2Generator::vertex_attr_definition(const std::string& prog_name, const StageAttr& attr) {
    return fmt::format("const i32 {} = {};", vertex_attr_name(prog_name, attr), attr.slot);
}

std::string SokolC2Generator::texture_bind_slot_definition(const Texture& tex) {
    return fmt::format("const i32 {} = {};", texture_bind_slot_name(tex), tex.sokol_slot);
}

std::string SokolC2Generator::sampler_bind_slot_definition(const Sampler& smp) {
    return fmt::format("const i32 {} = {};", sampler_bind_slot_name(smp), smp.sokol_slot);
}

std::string SokolC2Generator::uniform_block_bind_slot_definition(const UniformBlock& ub) {
    return fmt::format("const i32 {} = {};", uniform_block_bind_slot_name(ub), ub.sokol_slot);
}

std::string SokolC2Generator::storage_buffer_bind_slot_definition(const StorageBuffer& sbuf) {
    return fmt::format("const i32 {} = {};", storage_buffer_bind_slot_name(sbuf), sbuf.sokol_slot);
}

std::string SokolC2Generator::storage_image_bind_slot_definition(const StorageImage& simg) {
    return fmt::format("const i32 {} ({})", storage_image_bind_slot_name(simg), simg.sokol_slot);
}
