...
static void write_common_decls(slang_t::type_t slang, const args_t& args, const input_t& inp, const cross_t& cross) {
...
static void write_shader_desc_init(const char* indent, const program_t& prog, const input_t& inp, const cross_t& cross, const bytecode_t& bytecode, slang_t::type_t slang) {
    ...
    const cross_source_t& vs_src = cross.sources[vs_src_index];
    const cross_source_t& fs_src = cross.sources[fs_src_index];
...
static void write_attr_slot_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    assert(vs_src);
...
static void write_image_slot_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    const cross_source_t* fs_src = find_cross_source_by_shader_name(prog.fs_name, inp, cross);
    assert(vs_src && fs_src);
...
static void write_uniformblock_slot_stage(const cross_source_t* src) {
...
static void write_uniformblock_slot_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    const cross_source_t* fs_src = find_cross_source_by_shader_name(prog.fs_name, inp, cross);
    assert(vs_src && fs_src);
...
static void write_uniformblock_size_stage(const cross_source_t* src, const input_t& inp) {
    for (const uniform_block_t& ub: src->refl.uniform_blocks) {
        if (ub.slot >= 0) {
            L("    if (0 == strcmp(ub_name, \"{}\")) {{\n", ub.struct_name);
            L("      return sizeof({}{}_t);\n", mod_prefix(inp), ub.struct_name);
...
static void write_uniformblock_size_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    const cross_source_t* fs_src = find_cross_source_by_shader_name(prog.fs_name, inp, cross);
    assert(vs_src && fs_src);
...
static void write_uniform_offset_stage(const cross_source_t* src) {
    for (const uniform_block_t& ub: src->refl.uniform_blocks) {
        if (ub.slot >= 0) {
            L("    if (0 == strcmp(ub_name, \"{}\")) {{\n", ub.struct_name);
...
static void write_uniform_offset_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    const cross_source_t* fs_src = find_cross_source_by_shader_name(prog.fs_name, inp, cross);
    assert(vs_src && fs_src);
...
static void write_uniform_desc_stage(const cross_source_t* src) {
    for (const uniform_block_t& ub: src->refl.uniform_blocks) {
        if (ub.slot >= 0) {
            L("    if (0 == strcmp(ub_name, \"{}\")) {{\n", ub.struct_name);
            for (const uniform_t& u: ub.uniforms) {
...
static void write_uniform_desc_func(const program_t& prog, const args_t& args, const input_t& inp, const cross_t& cross) {
    const cross_source_t* vs_src = find_cross_source_by_shader_name(prog.vs_name, inp, cross);
    const cross_source_t* fs_src = find_cross_source_by_shader_name(prog.fs_name, inp, cross);
    assert(vs_src && fs_src);
...
errmsg_t output_sokol_t::gen(const args_t& args, const input_t& inp,
                             const std::array<cross_t,slang_t::NUM>& cross,
                             const std::array<bytecode_t,slang_t::NUM>& bytecode)
{
    ...
    for (int i = 0; i < slang_t::NUM; i++) {
        ...
        if (args.slang & slang_t::bit(slang)) {
            ...
            if (!common_decls_written) {
                ...
                write_common_decls(slang, args, inp, cross[i]);
            }
            if (!guard_written) {
                guard_written = true;
    ...
    for (const auto& item: inp.programs) {
        const program_t& prog = item.second;
        write_shader_desc_func(prog, args, inp, cross, bytecode);
        if (args.reflection) {
            int slang_index = (int)slang_t::first_valid(args.slang);
            assert((slang_index >= 0) && (slang_index < slang_t::NUM));
            write_attr_slot_func(prog, args, inp, cross[slang_index]);
            write_image_slot_func(prog, args, inp, cross[slang_index]);
            write_uniformblock_slot_func(prog, args, inp, cross[slang_index]);
            write_uniformblock_size_func(prog, args, inp, cross[slang_index]);
            write_uniform_offset_func(prog, args, inp, cross[slang_index]);
            write_uniform_desc_func(prog, args, inp, cross[slang_index]);
        }
    }
