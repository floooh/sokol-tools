...
static bytecode_t mtl_compile(const args_t& args, const input_t& inp, const cross_t& cross, slang_t::type_t slang) {
    bytecode_t bytecode;
    std::string base_dir;
...
static bytecode_t d3d_compile(const input_t& inp, const cross_t& cross, slang_t::type_t slang) {
    bytecode_t bytecode;
...
bytecode_t bytecode_t::compile(const args_t& args, const input_t& inp, const cross_t& cross, slang_t::type_t slang) {
    ...
    if ((slang == slang_t::METAL_MACOS) || (slang == slang_t::METAL_IOS)) {
        bytecode = mtl_compile(args, inp, cross, slang);
    }
    #endif
    #if defined(_WIN32)
