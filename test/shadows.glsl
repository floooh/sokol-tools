...
//
float sampleShadowPCF(vec2 uv, vec2 smSize, float compare) {
    float result = 0.0;
    for (int x=-2; x<=2; x++) {
        for (int y=-2; y<=2; y++) {
            vec2 off = vec2(x,y)/smSize;
            result += sampleShadow(uv+off, compare);
        }
    }
    return result / 25.0;
}
...
void main() {
    ...
    if (n_dot_l > 0.0) {
        ...
        float s = sampleShadowPCF(smUV, shadowMapSize, depth);
